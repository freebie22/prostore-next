...
export async function addItemToCart(data: CartItem) {
  try {
    ...
    if (!cart) {
      ...
      return {
        success: true,
        message: `${product.name} was added successfully to your Cart`,
      };
    } else {
      ...
      return {
        ...
        message: `${product.name} ${
          existItem ? "updaated in" : "added to"
        } cart`,
      };
    }
  } catch (error: any) {
...
export async function getMyCart() {
  ...
  const cart = await prisma.cart.findFirst({
    where: { OR: [{ userId: userId }, { sessionCartId: sessionCartId }] },
  });
...
export async function removeItemFromCart(productId: string) {
  try {
    ...
    return {
      success: false,
      message: "An error occured while removing item from cart",
    };
  }
}
